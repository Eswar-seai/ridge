...
def clean_data(df):
    """Clean the DataFrame by removing rows with NaN or 'na' (case-insensitive)."""
    df_cleaned = df.dropna()  # Drop rows with NaN
...
class RidgeRegressionApp(QtWidgets.QWidget):
    ...
    def prepare_and_train_model(self, place_data, route_data):
        try:
            ...
            # Cleaning
            place_data_cleaned = clean_data(place_data_sorted)
            route_data_cleaned = clean_data(route_data_sorted)

            # Feature and target selection
            selected_columns = ['fanout', 'netcount', 'netdelay', 'invdelay', 'bufdelay', 'seqdelay', 'skew', 'combodelay', 'wirelength']
            target_column = 'slack'

            features = place_data_cleaned[selected_columns]
            target = route_data_cleaned[target_column]
