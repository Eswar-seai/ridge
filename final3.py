...
class DataProcessingApp(QtWidgets.QWidget):
    ...
    def clean_data_from_db(self):
        try:
            ...
            # Save cleaned files in the same directory
            place_file_cleaned = f"{place_table}_cleaned.csv"
            route_file_cleaned = f"{route_table}_cleaned.csv"
            self.cleaned_place_data.to_csv(place_file_cleaned, index=False)
            self.cleaned_route_data.to_csv(route_file_cleaned, index=False)

            QMessageBox.information(self, "Success", f"Data cleaned successfully.\nFiles saved as:\n{place_file_cleaned}\n{route_file_cleaned}")
    ...
    def train_model(self):
        try:
            ...
            QMessageBox.information(self, "Success", f"Model trained successfully.\nTraining R-squared Score: {train_r2:.2f}\nTest R-squared Score: {test_r2:.2f}")
